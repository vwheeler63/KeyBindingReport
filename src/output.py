...
``KeyBindingData`` class and generating tables of key-binding data
...
headings.  (Platform-specific key names and column headings are in the
``platform`` module.)
...
represent itself as a string in the desired format.  Generating this
representation is left to the caller---the lists of fully-populated
Footnote objects are returned with the tables they belong to.
...
A.  There is a concept of a Footnote object:  a container for
    key-binding table footnotes.

    1.  It has:
        +   a reference to the key binding whose context it represents,
        +   the footnote number it was assigned,
        +   flag bits (bit-wise OR-ed FlagBits class values) containing
        ...
        +   the requested data format (affects its output representation when
            the requested format was reStructuredText).
    2.  It can be asked:
...
B.  There is a concept of a output object.
